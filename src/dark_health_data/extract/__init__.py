...
def get_extractor(name: str, **kwargs) -> Extractor:
    ...
    ``dhd batch``), 'vision' (Claude vision read of figure/chart pages -- the decorrelated
    multimodal ensemble member), or 'vlm' (any OpenAI-compatible/local model, e.g. Qwen)."""
    if name == "rule":
    ...
    if name == "vision":
        from .vision import ClaudeVisionExtractor  # lazy: optional dependency
    ...
    raise ValueError(
        f"Unknown extractor '{name}'. Use 'rule', 'llm', 'llm_batch', 'vision', or 'vlm'."
    )
